...
TODO:
1. Познакомиться с ресурсом: https://www.agilealliance.org/agile101/the-agile-manifesto/
2. Scrum:	https://scrumguides.org/docs/scrumguide/v2020/2020-Scrum-Guide-US.pdf#zoom=100
3. RUP:		https://public.dhe.ibm.com/software/rational/web/datasheets/version6/rup.pdf

TODO:
1. Обеспечить одновременное подключение нескольких клиентов к серверу;	DONE
2. Отобразить на сервере количество подключенных клиентов;				DONE
3. Обеспечить закрытие потоков при отключении клиентов;					DONE
4. Разобраться почему подключение на свободный слот выполняется с задержкой;
5. Вывести время сообщения на клиенте и на сервере;

DONE:
1. При подключении клиента к серверу сервер должен выводить на экран IP-адрес и порт клиента;	DONE
2. Сообщение, отправляемое на Сервер должен вводить пользователь с клавиатуры;					DONE
